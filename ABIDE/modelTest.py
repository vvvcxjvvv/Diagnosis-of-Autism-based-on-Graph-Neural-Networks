...
subjectNums = np.genfromtxt(r'F:\demo\GCN\GCN_ASD\ABIDE\subject_IDs_young18.txt', dtype=int)

model = torch.load(r'F:\demo\GCN\GCN_ASD\ABIDE\model\ASD_74.pth')
out2 = model(feature, support_mid)
pred = out2.argmax(dim=1)

correct = 0
for i in range(len(test_ind)):
    if y_data[test_ind[i]] == 0:
        label_test = "NC"
    else:
        label_test = "ASD"

    if pred[test_ind[i]] == 0:
        predict = "NC"
    else:
        predict = "ASD"

    if y_data[test_ind[i]] == pred[test_ind[i]]:
        result = "True"
        correct = correct + 1
    else:
        result = "False"

    print("subject:{} label:{} predict:{} result:{}".format(subjectNums[test_ind[i]], label_test, predict, result))



print("Number of samples:{}".format(i+1))
print("Accuracy:{}".format(correct/len(test_ind)))
